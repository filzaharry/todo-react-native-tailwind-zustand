...
type Action = {
  ...
  submitUpdateTask: (data: TaskSubmitType) => void;
};
...
const zustandBase = create<State & Action>((set) => ({
  ...
  submitUpdateTask: async (data: TaskSubmitType) => {
    // get async storage taskData
    const getTask: any = await AsyncStorage.getItem("taskData");
    // AsyncStorage.removeItem("taskData");

    // delete item submit
    const getAllTask: any = await AsyncStorage.getItem("taskData");
    const parseData = JSON.parse(getAllTask)
